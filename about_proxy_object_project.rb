...
class Proxy
  ...
  def initialize(target_object)
	...
	def method_missing(method, *args, &block) 
		if @wiadomosc.keys.include?(method) == true
			@wiadomosc[method] += 1
		else
			@wiadomosc[method] = 1
		end
	...
	def called?(method)
    @wiadomosc[method] != nil
  end

  def number_of_times_called(method)
		if @wiadomosc.keys.include?(method)
			@wiadomosc[method]
		else
			0
		end
	...
	def no_such_method
		raise NoMethodError, "bo tak ma byc"
	end
...
class AboutProxyObjectProject < Neo::Koan
  ...
  def test_proxy_counts_method_calls
    ...
    assert_equal 1, tv.number_of_times_called(:channel=)
		print tv.number_of_times_called(:on?)
